...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif (!$newPwd || $newPwd !== $confirm) {
        $error = 'New passwords do not match.';
    } else {
        $pdo->prepare(
          'UPDATE user_master SET password = ?, updated_at = NOW()
...
<body>
    <div class="card-box">
        ...
        <form method="post" class="text-start">
            ...
            <input type="text" name="new_pwd" class="form-control mb-3" required>

            <label class="form-label mb-1">Confirm New Password</label>
            <input type="text" name="confirm_pwd" class="form-control mb-4" required>

            <button class="btn btn-blue py-2">Update Password</button>
        </form>
