...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['withdraw_id'])) {
    ...
    // ✅ FIX: Use null coalescing to avoid warnings
    $status = $_GET['status'] ?? '';
    $page   = $_GET['p'] ?? 1;
    header("Location: intern_dashboard.php?status=$status&p=$page");
    exit;
}
...
if ($statusFilter) $countSql .= " AND status = :status";
$totalRows = $pdo->prepare($countSql);
$totalRows->bindValue(':id', $userId, PDO::PARAM_INT);
if ($statusFilter) $totalRows->bindValue(':status', $statusFilter);
$totalRows->execute();
...
$stats = $stat->fetch(PDO::FETCH_ASSOC) ?: ['approved'=>0, 'pending'=>0, 'rejected'=>0];

/* Paginated Gatepasses */
$sql = "
  SELECT gatepass_id, reason, date_for, from_time, to_time, status
  FROM gatepasses
  WHERE user_id = :id AND delflag = 0
";
if ($statusFilter) $sql .= " AND status = :status";
$sql .= " ORDER BY created_at DESC LIMIT :per OFFSET :off";

$list = $pdo->prepare($sql);
$list->bindValue(':id',  $userId, PDO::PARAM_INT);
if ($statusFilter) $list->bindValue(':status', $statusFilter);
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h3 class="fw-semibold m-0">Welcome <?= htmlspecialchars($intern['first_name']) ?>!</h3>
...
<div class="d-flex flex-wrap gap-3 mb-4">
    ...
    <a href="intern_dashboard.php" class="text-decoration-none">
        <div
            ...
            <tbody>
                ...
                <tr class="status-<?= $g['status'] ?>">
                    ...
                    <td>
                        ...
                        <form method="post" onsubmit="return confirm('Withdraw this gatepass request?');">
